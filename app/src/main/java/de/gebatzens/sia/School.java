...
public class School {
    ...
    public void loadTheme() {
        String name = SIAApp.SIA_APP.getCustomThemeName();
        if(name == null)
            name = themeName;
        theme = SIAApp.SIA_APP.getResources().getIdentifier("AppTheme" + name, "style", SIAApp.SIA_APP.getPackageName());
        colorArray = SIAApp.SIA_APP.getResources().getIdentifier("CardviewColor" + name, "array", SIAApp.SIA_APP.getPackageName());
        TypedArray ta = SIAApp.SIA_APP.obtainStyledAttributes(theme, new int [] {R.attr.colorPrimary});
        TypedArray tad = SIAApp.SIA_APP.obtainStyledAttributes(theme, new int [] {R.attr.colorPrimaryDark});
        TypedArray taa = SIAApp.SIA_APP.obtainStyledAttributes(theme, new int [] {R.attr.colorAccent});
    ...
    public static boolean fetchList() {
        ...
        SIAApp.SIA_APP.getResources().getIdentifier("asd", "styles", SIAApp.SIA_APP.getPackageName());

        List<School> newList = new ArrayList<>();

        try {
            SiaAPI.APIResponse re = SIAApp.SIA_APP.api.doRequest("/getSchools", null);
    ...
    public static void loadList() {
        LIST.clear();
        try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(SIAApp.SIA_APP.openFileInput("schools")));
            String content = "", line = "";
    ...
    public Bitmap loadImage() {
        try {
            return BitmapFactory.decodeStream(SIAApp.SIA_APP.openFileInput(image));
        } catch (Exception e) {
            ...
            return BitmapFactory.decodeResource(SIAApp.SIA_APP.getResources(), R.drawable.no_content);
        }
    }
    ...
    public static boolean downloadImage(String image) {
        ...
        try {
            ...
            BitmapFactory.decodeStream(in).compress(Bitmap.CompressFormat.PNG, 90, SIAApp.SIA_APP.openFileOutput(image, Context.MODE_PRIVATE));
            in.close();
            return true;
        } catch(Exception e) {
            Log.e("ggvp", e.toString());
